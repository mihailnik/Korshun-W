...
void create_parameters(){
    ...
    */
    ...
    embui.section_handle_add(FPSTR(T_MIUI), block_mipage);                // generate "Demo" UI section

    // обработчики
    embui.section_handle_add(FPSTR(V_LED), action_blink);               // обработка рычажка светодиода
    embui.section_handle_add(FPSTR(V_BLINK_LED), action_miblink);               // обработка рычажка светодиода
    embui.section_handle_add(FPSTR(V_UPDRATE), setRate);                 // sensor data publisher rate change
    embui.section_handle_add(FPSTR(V_UPDVOL), setVolume);                 // sensor data publisher rate change
    embui.section_handle_add(FPSTR(V_UPDVOICE), setVoice);                 // sensor data publisher rate change
...
void block_menu(Interface *interf, JsonObject *data){
  ...
  interf->option(FPSTR(T_MIUI), F("Mi UI"));
...
void block_mipage(Interface *interf, JsonObject *data){
    ...
    // параметр FPSTR(T_SET_DEMO) определяет зарегистрированный обработчик данных для секции
    LOG(println, F("ESP8266"));
    interf->json_section_main(FPSTR(T_SET_MIUI), F("KORSHUN-W16"));

    // переключатель, связанный со светодиодом. Изменяется синхронно
    interf->checkbox(FPSTR(V_BLINK_LED), F("blink LED"), true);

    interf->comment(F("Comment: some mi sensors"));     // комментарий-описание секции
    ...
    // Fake temperature sensor
    interf->display(F("temp"), String(24));
    interf->display(F("temp"), "test_display");
    interf->json_section_end();     // end of line
    ...
    // Update Volume and Voice slider
    interf->range(FPSTR(V_UPDVOL), String(tDisplayUpdater.getInterval()/1000), String(1), String(30), String(1), String(F("Volume:")), true);
    interf->range(FPSTR(V_UPDVOICE), String(tDisplayUpdater.getInterval()/1000), String(1), String(30), String(1), String(F("Volume:")), true);
...
/**
 * Change update Volume callback
 */
void setVolume(Interface *interf, JsonObject *data) {
  if (!data) return;
  // tChange.setVolume( (*data)[FPSTR(V_UPDVOL)].as<char>()); // set update rate in seconds
}

/**
 * Change update Voice callback
 */
void setVoice(Interface *interf, JsonObject *data) {
  if (!data) return;
  //setVoi((*data)[FPSTR(V_UPDVOICE)].as<char>());
}

/**
 * Change update Timer callback
 */
...
void action_miblink(Interface *interf, JsonObject *data){
  ...
  SETPARAM(FPSTR(V_BLINK_LED));  // save new LED state to the config
